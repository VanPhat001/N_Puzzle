...
void write_lastMap_inFile(int map[][MAX_N], int write_map) // default: rows = cols = n
{
...
void print_text_in_coord(int y, int x, char * text) // y == -1 || x == -1 --> align center
{
	if (x == -1)
	{
...
void loading()
{
	set_console_size(102, 6);
	print_border(0, 0, 1, 1, 100); // in khung theo format loading
	...
	print_text_in_coord(4, -1, "Press any key to start...");
	getch();
}
...
//	readMap(); // test case
	...
	print_border(-1, -1, n, n, WIDTH); // in khung theo format map
	print_map();
}
	...
	switch (key)
	{
		case 0: // play game
			...
			switch (play_again())
			{
				...
				case 1: // solve map
					solve_map();
					write_lastMap_inFile(mapdau, 0);

					int startX = n - 1, startY = n - 1;
					chuyen_toado(&startX, &startY);
					textcolor(GREEN);
					print_text_in_coord(startY + 3, -1, "Complete!");
					getch();
