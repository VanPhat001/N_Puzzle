...
//			int color = (vitri.x == j && vitri.y == i) ? GREEN : WHITE;
			color = (arr[i][j] == i*n + j + 1 ? LIGHTCYAN : WHITE);
...
void print_map()
{
	...
	for (i = 0; i < n; i++)
	{
		...
		{
			int color = (vitri.x == j && vitri.y == i) ? GREEN : WHITE;
			color = (mapdau[i][j] == i*n + j + 1 ? LIGHTCYAN : color);
...
//	set_console_size(SCREEN_MAX_X, SCREEN_MAX_Y);
	...
	while (1)
	{
		if (kbhit())
		{
			...
			int color = (mapdau[vitri.y][vitri.x] == vitri.y*n + vitri.x + 1 ? LIGHTCYAN : WHITE);
