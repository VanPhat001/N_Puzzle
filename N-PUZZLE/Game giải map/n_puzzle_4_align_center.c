...
{
	int startX = SCREEN_MAX_X/2 - (n*(WIDTH + 1) + 1)/2;
	int startY = SCREEN_MAX_Y/2 - (n*(HEIGHT + 1) + 1)/2;
	*x = startX + *x * (WIDTH + 1) + 2;
	*y = startY + *y * (HEIGHT + 1) + 1 - 3;
}

void print_border(int row, int col, int width) // height = 1
{
	int hang, cot, startX = 0, startY = 0;
...
{
	int i, j, color = WHITE, startX = 0, startY = 0;
	chuyen_toado(&startX, &startY);
	for (i = 0; i < n; i++)
	{
...
void play_game()
{
	...
	while (1)
	{
		if (kbhit())
		{
			...
			print_number(mapdau[vitri.y][vitri.x] ,vitri.x, vitri.y, WHITE); // xoa trang thai cu
			...
			switch (c)
			{
				case 13:
					...
					if (ktra_ketthuc())
					{
						...
						gotoxy(SCREEN_MAX_X/2 - 9, startY + 4);
						printf("YOU WIN\n");
						gotoxy(SCREEN_MAX_X/2 - 9, startY + 5);
						printf("Time play  : %02d:%02d\n", timePlay/60, timePlay%60);
						gotoxy(SCREEN_MAX_X/2 - 9, startY + 6);
		...
		textcolor(WHITE);
		chuyen_toado(&startX, &startY);
		gotoxy(SCREEN_MAX_X/2 - 16, startY + 3);
		printf("Cursor: (%02d,%02d)  ---  Time: %02d:%02d", vitri.y, vitri.x, timePlay/60, timePlay%60);
	}
	out_loop:;
	getch();
}
...
void ids_final_problem(int last_map[][MAX_N]) // from mapdau become last_map
{
	...
	chuyen_toado(&startX, &startY);
	gotoxy(SCREEN_MAX_X/2 - 9, startY + 3);
	...
	gotoxy(SCREEN_MAX_X/2 - 16, startY + 4);
		...
		case 0: // play game
			...
			switch (play_again())
			{
				...
				case 1: // solve map				
					...
					chuyen_toado(&startX, &startY);
					textcolor(GREEN);
					gotoxy(SCREEN_MAX_X/2 - 5, startY + 3);
					printf("Complete!");
					getch();
