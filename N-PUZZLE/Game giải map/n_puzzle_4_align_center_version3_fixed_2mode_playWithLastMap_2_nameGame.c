...
{
	int startY = 3;
	set_console_size(162, 20 + startY);
	...
	textcolor(WHITE);
	print_border(11 + startY, 30, 1, 1, 100); // in khung theo format loading

	int i;
	for (i = 1; i <= 100; i++)
	{
		gotoxy(i + 30, 12 + startY);
		textcolor(GREEN);
		printf("%c", 219); // 221
		gotoxy(74, 15 + startY);
		textcolor(WHITE);
...
//	set_console_size(SCREEN_MAX_X, SCREEN_MAX_Y);
	...
	while (1)
	{
		...
		textcolor(WHITE);
		chuyen_toado(&startX, &startY);
		gotoxy(getConsoleSize_Cols()/2 - 16, startY + 3);
